...
function buildAgentPhases(events: AgentEvent[]): AgentPhase[] {
  ...
  for (const agent of agentOrder) {
    ...
    const progressEvents = agentEvents.filter((e) => e.progress_percent != null);
    const maxProgress = progressEvents.length > 0 ? Math.max(...progressEvents.map((e) => e.progress_percent!)) : 0;
...
function ProgressBar({ progress, isActive, isComplete, isError, color }: { progress: number; isActive: boolean; isComplete: boolean; isError: boolean; color: string }) {
...
function ExpandableSection({ label, children }: { label: string; children: React.ReactNode }) {
  ...
  return (
    <div className="mt-1">
      ...
      {open && <div className="mt-1 rounded border border-gray-600 bg-gray-800 p-2 text-xs text-gray-300 overflow-auto max-h-40">{children}</div>}
    </div>
  );
}
...
function AgentCard({ phase }: { phase: AgentPhase }) {
  ...
  const statusIcon = phase.isComplete ? "\u2705" : phase.isError ? "\u274C" : phase.isWarning ? "\u26A0\uFE0F" : phase.isActive ? "\u23F3" : "\u23ED";

  return (
    <div className={`rounded border ${phase.isActive ? `${borderColor} ring-1 ring-opacity-30` : "border-gray-700"} bg-gray-900 p-3 transition-all`}>
      <div className="flex items-center justify-between gap-2">
        ...
        <span className={`flex-shrink-0 text-xs ${phase.isError ? "text-red-400" : phase.isComplete ? "text-green-400" : "text-gray-400"}`}>
          {phase.isComplete ? "Done" : phase.isError ? "Failed" : phase.isActive ? `${phase.progress}%` : "Waiting"}
        </span>
      </div>

      <ProgressBar progress={phase.progress} isActive={phase.isActive} isComplete={phase.isComplete} isError={phase.isError} color={color} />

      {phase.status && (phase.isActive || phase.isComplete || phase.isError) && (
        <p className={`mt-1 text-xs ${phase.isError ? "text-red-400" : "text-gray-400"}`}>{phase.status}</p>
      )}

      {phase.isComplete && phase.artifact && (
        <ExpandableSection label="Show details">
          <pre className="whitespace-pre-wrap break-all">{JSON.stringify(phase.artifact, null, 2)}</pre>
        </ExpandableSection>
      )}
      ...
      {phase.isActive && phase.partialText && (
        <ExpandableSection label="Show intermediate output">
          <pre className="whitespace-pre-wrap break-all">{phase.partialText}</pre>
        </ExpandableSection>
      )}
    </div>
  );
...
export default function AgentTimeline({ events, connected, reconnecting, isPolling, visible }: Props) {
    ...
    <div className="w-full max-w-5xl space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className={`inline-block h-2 w-2 rounded-full ${connected ? "bg-green-500" : reconnecting ? "bg-yellow-500 animate-pulse" : "bg-red-500"}`} />
          <span className="text-xs text-gray-500">
            {connected ? "Live" : reconnecting ? "Reconnecting..." : isPolling ? "Polling" : "Disconnected"}
          </span>
        </div>
        <button
